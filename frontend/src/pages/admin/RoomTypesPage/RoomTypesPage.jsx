...
import RoomTypeImagesModal from '../../../components/manager/RoomTypeImagesModal/RoomTypeImagesModal';
...
export default function RoomTypesPage() {
  ...
  const [roomTypeForImages, setRoomTypeForImages] = useState(null);
  ...
  const handleToggle = () => {
    const id = selectedRoomType.room_type_id || selectedRoomType.id;
    const newStatus = selectedRoomType.status === 'active' ? 'inactive' : 'active';
    managerApi
      .updateRoomTypeStatus(id, { status: newStatus })
      .then(() => {
  ...
  const columns = [
    ...
    { key: 'total_rooms', label: 'Total Rooms' },
    {
      ...
      label: 'Status',
      render: (rt) => (
        <Badge variant={rt.status === 'active' ? 'success' : 'error'}>{rt.status}</Badge>
      ),
    },
    {
      key: 'actions',
      ...
      render: (rt) => (
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          <Button
            variant={rt.status === 'active' ? 'ghost' : 'primary'}
            size="sm"
            onClick={() => {
  ...
  return (
    <div className={styles.container}>
      <PageHeader
        title="Room Types"
        description="Manage your hotel's room types and their photo galleries."
        action={{ label: 'Add room type', onClick: () => setModalOpen(true) }}
      />
      <Table
        columns={columns}
        data={roomTypes}
        emptyMessage="No room types yet. Click 'Add room type' to get started."
      />
      ...
      {/* Room Type Images Modal */}
      <RoomTypeImagesModal
        isOpen={imageModalOpen}
        onClose={() => {
          setImageModalOpen(false);
          setRoomTypeForImages(null);
        }}
        roomType={roomTypeForImages}
      />
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to ${
          selectedRoomType?.status === 'active' ? 'deactivate' : 'activate'
